...
ssize_t z_impl_hwinfo_get_device_id(uint8_t *buffer, size_t length)
{
	...
	dev_id.id[0] = sys_cpu_to_be32(LL_GetUID_Word1());
	dev_id.id[1] = sys_cpu_to_be32(LL_GetUID_Word0());
...
#define CHECK_RST_FLAG(flag) (RAM_VR.ResetReason == (flag | RCC_FLAG_PADRSTF))
int z_impl_hwinfo_get_reset_cause(uint32_t *cause)
{
	uint32_t flags = 0;

#if defined(RCC_FLAG_SFTRST)
	if (CHECK_RST_FLAG(RCC_FLAG_SFTRST)) {
		flags |= RESET_SOFTWARE;
	}
#endif
#if defined(RCC_FLAG_PADRSTF)
	if (CHECK_RST_FLAG(RCC_FLAG_PADRSTF)) {
		flags |= RESET_PIN;
	}
#endif
#if defined(RCC_FLAG_WDGRST)
	if (CHECK_RST_FLAG(RCC_FLAG_WDGRST)) {
		flags |= RESET_WATCHDOG;
	}
#endif
#if defined(RCC_FLAG_LOCKUPRST)
	if (CHECK_RST_FLAG(RCC_FLAG_LOCKUPRST)) {
		flags |= RESET_CPU_LOCKUP;
	}
#endif
	if (CHECK_RST_FLAG(0) && (PWR->EXTSRR & PWR_EXTSRR_DEEPSTOPF) && (SYSCFG->PWRC_ISCR & SYSCFG_PWRC_ISCR_WKUP_ISC)) {
	...
	if (CHECK_RST_FLAG(RCC_FLAG_PORRST)) {
		flags |= RESET_POR;
	}
#endif
