...
#elif defined (CONFIG_SOC_SERIES_BLUENRG_3)
	LL_EXTI_EnableIT(BIT((uint32_t)line));
#else
...
#elif defined (CONFIG_SOC_SERIES_BLUENRG_3)
	LL_EXTI_DisableIT(BIT((uint32_t)line));
#else
...
#elif defined (CONFIG_SOC_SERIES_BLUENRG_3)
		return LL_EXTI_IsInterruptPending(BIT((uint32_t)line));
#else
...
#elif defined (CONFIG_SOC_SERIES_BLUENRG_3)
		LL_EXTI_ClearInterrupt(BIT((uint32_t)line));
#else
...
void stm32_exti_trigger(int line, int trigger)
{
	...
	LL_EXTI_SetTrigger(trigger, BIT(line));
	z_stm32_hsem_unlock(CFG_HW_EXTI_SEMID);
}
...
{
	for (int i = 0; i < len; i++) {
		exti_irq_table[start + i] = 15 + (irqn % 16);
	}
}
