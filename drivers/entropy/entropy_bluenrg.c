...
#define STM32_RNG_THREAD_STACK_SIZE 256
#define STM32_RNG_THREAD_PRIO 10
/*
...
struct entropy_stm32_rng_dev_data {
	...
	struct k_sem sem_low_threshold;
	bool filling_pools;
...
static K_KERNEL_STACK_DEFINE(stm32_rng_thread_stack, STM32_RNG_THREAD_STACK_SIZE);
static struct k_thread stm32_rng_thread_data;
...
#endif
	if ((LL_RNG_IsActiveFlag_RNGRDY(rng) == 1)) {
		if (entropy_stm32_got_error(rng)) {
			retval = -EIO;
			goto out;
		}

		retval = LL_RNG_ReadRandData16(rng);
		if (retval == 0) {
...
static int start_pool_filling(bool wait)
{
	...
	acquire_rng();
	k_sem_give(&entropy_stm32_rng_data.sem_low_threshold);
...
static void stm32_rng_thread(void)
{
	int byte, ret;

	while (true) {

	    k_sem_take(&entropy_stm32_rng_data.sem_low_threshold, K_FOREVER);

	    while (entropy_stm32_rng_data.filling_pools) {
					     ...
					     uint32_t flags)
{
	...
	cnt = rng_pool_get((struct rng_pool *)(entropy_stm32_rng_data.isr),
			   buf, len);
	if (flags & ENTROPY_BUSYWAIT) {
		...
		value = LL_RNG_ReadRandData16(rng);
		buf[cnt++] = value & 0xff;
		if ((len - cnt) >= 2) {
...
static int entropy_stm32_rng_init(const struct device *dev)
{
	...
	/* Filling semaphore */
	k_sem_init(&dev_data->sem_low_threshold, 0, 1);
	...
	/* Start RNG filler thread */
	k_thread_create(&stm32_rng_thread_data, stm32_rng_thread_stack,
			K_KERNEL_STACK_SIZEOF(stm32_rng_thread_stack),
			(k_thread_entry_t)stm32_rng_thread, NULL, NULL, NULL,
			K_PRIO_COOP(STM32_RNG_THREAD_PRIO),
			0, K_NO_WAIT);
	k_thread_name_set(&stm32_rng_thread_data, "Entropy");
