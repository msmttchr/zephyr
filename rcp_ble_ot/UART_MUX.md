...
The multiplexer is implemented at **application level** and does not
...
### 2.1 What problem does the UART mux solve?
...
### 2.3 Configuration via Devicetree

Virtual UARTs are configured entirely in Devicetree.

Key properties include:
- **Priority**  
  Determines TX arbitration order  
...
### 2.5 What the user does *not* need to handle
...
Only one frame is transmitted at a time on the physical UART.

TX arbitration between channels is priority-based.
...
Frames with invalid CRC or unknown PLC values are dropped.
...
The UART multiplexer was designed with the following goals:
